...
# различное число ягод – на i-ом кусте выросло ai ягод. В этом фермерском хозяйстве внедрена система автоматического сбора черники.
...
garden_bed = []
for _ in range(shrub):
    garden_bed.append(randint(0, 9))
print(garden_bed)

max = garden_bed[0] + garden_bed[1] + garden_bed[2]
for i in range(shrub - 1):
    if garden_bed[i - 1] + garden_bed[i] + garden_bed[i + 1] > max:
        max = garden_bed[i - 1] + garden_bed[i] + garden_bed[i + 1]
print(f'Макимальное количество год: {max}')
